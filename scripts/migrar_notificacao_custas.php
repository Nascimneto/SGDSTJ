<?php
/**
 * Migração: adiciona os campos "Notificação de Conta e Custas" e
 * "Notificação 2ª Conta e Custas" (datas_controlo.notificacao_conta_custas/2)
 * e actualiza a vista v_processos_completos para os expor. Não apaga nem
 * altera dados existentes.
 *
 * Só pode ser executado via CLI: php scripts/migrar_notificacao_custas.php
 */
if (PHP_SAPI !== 'cli') {
...
try {
    if (!sgd_tem_coluna($pdo, $bd, 'datas_controlo', 'notificacao_conta_custas')) {
        $pdo->exec(
            "ALTER TABLE datas_controlo
             ADD COLUMN notificacao_conta_custas DATE NULL
             AFTER conta_custas2"
        );
        echo "✓ datas_controlo.notificacao_conta_custas criada.\n";
    } else {
        echo "  datas_controlo.notificacao_conta_custas já existe (ignorado).\n";
    }

    if (!sgd_tem_coluna($pdo, $bd, 'datas_controlo', 'notificacao_conta_custas2')) {
        $pdo->exec(
            "ALTER TABLE datas_controlo
             ADD COLUMN notificacao_conta_custas2 DATE NULL
             AFTER notificacao_conta_custas"
        );
        echo "✓ datas_controlo.notificacao_conta_custas2 criada.\n";
    } else {
        echo "  datas_controlo.notificacao_conta_custas2 já existe (ignorado).\n";
    }
