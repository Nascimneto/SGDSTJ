<?php
/**
 * Migração: adiciona o campo "Data de Redistribuição"
 * (datas_controlo.redistribuicao_data) e actualiza a vista
 * v_processos_completos para o expor. Distinto de processos.redistribuicao,
 * que guarda apenas o nome do novo magistrado. Não apaga nem altera dados
 * existentes.
 *
 * Só pode ser executado via CLI: php scripts/migrar_redistribuicao_data.php
 */
if (PHP_SAPI !== 'cli') {
...
try {
    if (!sgd_tem_coluna($pdo, $bd, 'datas_controlo', 'redistribuicao_data')) {
        $pdo->exec(
            "ALTER TABLE datas_controlo
             ADD COLUMN redistribuicao_data DATE NULL
             AFTER processo_id"
        );
        echo "✓ datas_controlo.redistribuicao_data criada.\n";
    } else {
        echo "  datas_controlo.redistribuicao_data já existe (ignorado).\n";
    }
