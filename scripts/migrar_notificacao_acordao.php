<?php
/**
 * Migração: adiciona os campos "Notificação do 2º Acórdão" e "Notificação
 * do 3º Acórdão" (datas_controlo.notificacao_acordao2/3) e actualiza a
 * vista v_processos_completos para os expor. Não apaga nem altera dados
 * existentes.
 *
 * Só pode ser executado via CLI: php scripts/migrar_notificacao_acordao.php
 */
if (PHP_SAPI !== 'cli') {
...
try {
    if (!sgd_tem_coluna($pdo, $bd, 'datas_controlo', 'notificacao_acordao2')) {
        $pdo->exec(
            "ALTER TABLE datas_controlo
             ADD COLUMN notificacao_acordao2 DATE NULL
             AFTER notificacao_acordao"
        );
        echo "✓ datas_controlo.notificacao_acordao2 criada.\n";
    } else {
        echo "  datas_controlo.notificacao_acordao2 já existe (ignorado).\n";
    }

    if (!sgd_tem_coluna($pdo, $bd, 'datas_controlo', 'notificacao_acordao3')) {
        $pdo->exec(
            "ALTER TABLE datas_controlo
             ADD COLUMN notificacao_acordao3 DATE NULL
             AFTER notificacao_acordao2"
        );
        echo "✓ datas_controlo.notificacao_acordao3 criada.\n";
    } else {
        echo "  datas_controlo.notificacao_acordao3 já existe (ignorado).\n";
    }
